...
1. What is the purpose of using _sessions_?
    ...
    Sessions keep information about a client, usually to maintain authentication so the client does not need to login again if the browser is closed and then they revisit.

2. What does bcrypt do to help us store passwords in a secure manner.

    Bcrypt hashes the password so that if someone gains access to the database they would not be able to easily see the users password which is likely used on multiple sites.

3. What does bcrypt do to slow down attackers?

    Bcrypt cryptographically hashes the inputed password so attackers can only easily view a hashed string. In order to reverse it they would need to have the parameters exactly correct.
